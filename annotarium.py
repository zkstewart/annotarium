...
from modules.validation import validate_ap, \
    ...
    validate_f, validate_f_explode, validate_f_gaps, validate_f_ids, validate_f_lengths, validate_f_rc, validate_f_rename, validate_f_slice, validate_f_softmask, validate_f_stats, \
...
from modules.fasta import fasta_explode, fasta_gaps_to_bed, fasta_ids, fasta_lengths, fasta_rc, fasta_rename, fasta_slice, fasta_softmask_to_bed, fasta_stats
...
def main():
    ...
    cparser = subparsers.add_parser("cluster",
                                    parents=[p],
                                    add_help=False,
                                    help="Clustering result (CD-HIT/MMseqs2/BINge) handling")
    cparser.set_defaults(func=cmain)
    
    subClusterParsers = cparser.add_subparsers(dest="clusterMode",
                                               required=True)
    
    # Cluster > reformat mode
    creformatparser = subClusterParsers.add_parser("reformat",
                                                   parents=[p],
                                                   add_help=False,
                                                   help="Reformat and/or filter clustering predictions")
    creformatparser.add_argument("-i", dest="clusterFile",
                                 required=True,
                                 help="Location of any compatible clusterer's result file")
    creformatparser.add_argument("-if", dest="inputFileFormat",
                                 required=True,
                                 choices=["binge", "cdhit", "corset", "mmseqs", "pantools", "2columnleft", "2columnright"],
                                 help="""File format/program results being input; '2columnleft' means cluster IDs/keys
                                 are in the left column and member IDs in the right column""")
    creformatparser.add_argument("-o", dest="outputFileName",
                                 required=False,
                                 help="Write reformatted file to this location")
    creformatparser.add_argument("-of", dest="outputFileFormat",
                                 required=True,
                                 choices=["binge", "cdhit", "corset", "mmseqs", "pantools", "2columnleft", "2columnright"],
    ...
    # FASTA > ids mode
    flenparser = subFASTAParsers.add_parser("ids",
                                            parents=[p],
                                            add_help=False,
                                            help="Write sequence identifiers")
    ...
    flenparser.add_argument("--descriptions", dest="asDescription",
                            required=False,
                            action="store_true",
                            help="""Optionally write the full FASTA sequence header
                            ('description') rather than just the ID (which is everything
    ...
    flenparser = subFASTAParsers.add_parser("lengths",
                                            parents=[p],
                                            add_help=False,
                                            help="Write sequence lengths as TSV or list")
    flenparser.add_argument("-i", dest="fastaFile",
                            required=True,
                            help="Location of FASTA file")
    flenparser.add_argument("-o", dest="outputFileName",
    ...
    # FASTA > slice mode
    fsliceparser = subFASTAParsers.add_parser("slice",
                                              parents=[p],
                                              add_help=False,
                                              help="Slice region(s) from FASTA contig(s)")
    fsliceparser.add_argument("-i", dest="fastaFile",
                              required=True,
                              help="Location of FASTA file")
    fsliceparser.add_argument("-s", dest="slices",
                              required=True,
                              nargs="+",
                              help="One or more slices in contig:start-end format")
    fsliceparser.add_argument("-o", dest="outputFileName",
                              required=False,
                              help="Write FASTA slices to file")
    ...
    if args.mode == "cluster":
        print("## annotarium.py - Cluster handling ##")
        validate_c(args)
        cmain(args)
...
def cmain(args):
    # Split into sub-mode-specific functions
    if args.clusterMode == "reformat":
        print("## Clustering results reformatting ##")
        validate_c_reformat(args)
        cluster_reformat(args)
    
    print("Cluster handling complete!")
...
def fmain(args):
    ...
    if args.fastaMode == "ids":
        print("## FASTA identifiers ##")
        validate_f_ids(args)
        fasta_ids(args)
    ...
    if args.fastaMode == "rename":
        ...
        fasta_rename(args)
    if args.fastaMode == "slice":
        print("## FASTA slicing ##")
        validate_f_slice(args) # sets args.slices with updated formatting
        fasta_slice(args)
